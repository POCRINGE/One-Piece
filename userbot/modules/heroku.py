...
async def dyno_usage(dyno):
    ...
    """
    await dyno.edit("`Mendapatkan Tentang Dyno Sensei⚡`")
    useragent = (
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(heroku_api + path, headers=headers) as r:
            if r.status != 200:
                ...
                await dyno.edit("`Tidak Bisa Mendapatkan Informasi Dyno Sensei ☄`")
                return False
            result = await r.json()
...
async def _(dyno):
    ...
    await dyno.edit("`Sedang Mengambil Logs Milik Sensei`")
    ...
    await dyno.edit(f"`Ini Logs Heroku Anda Sensei:`\n\nPaste Ke: [Nekobin]({url})")
