...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("【Boker】"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**ꜱᴇɴꜱᴇɪ Sudah Siap Berak!**")
        time.sleep(3)
        await msg.delete()
        ...
        if BOTLOG:
            await notafk.client.send_message(
                ...
                str(len(USERS)) + " Obrolan Saat Anda Lagi Berak",
            )
            for i in USERS:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**☹ ꜱᴇɴꜱᴇɪ {ALIVE_NAME} Lagi Berak** {afk_since} **Yang Lalu.**\
                        \n☛ **Alasan :** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**✘ ꜱᴇɴꜱᴇɪ Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **Alasan ꜱᴇɴꜱᴇɪ:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"✘ **ꜱᴇɴꜱᴇɪ Lagi Berak** {afk_since} **Yang Lalu**.\
                        \n☛ **Alasan**: `{AFKREASON}`")
                else:
